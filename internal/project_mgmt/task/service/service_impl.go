...
func (s *TaskService) ListTasks(ctx context.Context, limit, offset int) ([]dto.TaskResponse, int64, error) {
	rows, err := s.repository.ListTasks(ctx, limit, offset)
	if err != nil {
		return nil, 0, err
	}
	total, err := s.repository.CountTasks(ctx)
	if err != nil {
		return nil, 0, err
	}
