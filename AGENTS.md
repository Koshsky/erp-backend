...
## Architecture
- Layered per domain: `internal/<domain>/{delivery,service,repository,domain,dto}`. Handlers (`delivery`) expose `RegisterRoutes(*gin.RouterGroup)` and implement the `RouteRegistrar` interface; every new handler/service must be wired in `internal/app/routes.go` (and its interface asserted in `services_check.go`).
...
## DB & migrations
- Postgres 16; schema is Flyway migrations in `migrations/` (V1..V10, applied by the `flyway` docker service). New schema changes = new `V*__name.sql`; never edit applied migrations.
